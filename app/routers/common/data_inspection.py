...
from app.models.fire_models import ProductBasicRate, StfiRate, EqRate, FireTerrorismRate, AddOnMaster, AddOnProductMap
...
def get_basic_rates(db: Session = Depends(get_db)):
    ...
    res = [{"id": r.id, "product_code": r.product_code, "rate": r.basic_rate} for r in data]
...
def get_stfi_rates(db: Session = Depends(get_db)):
    ...
    res = [{"id": r.id, "product_code": r.product_code, "rate": r.stfi_rate} for r in data]
...
def get_eq_rates(db: Session = Depends(get_db)):
    ...
    res = [{"id": r.id, "product_code": r.product_code, "zone": r.eq_zone, "rate": r.eq_rate} for r in data]
...
def get_terr_slabs(db: Session = Depends(get_db)):
    data = db.query(FireTerrorismRate).all()
    res = [{"id": r.id, "occupancy_type": r.occupancy_type, "min_si": r.min_sum_insured, "max_si": r.max_sum_insured, "rate": r.rate_per_mille} for r in data]
    return ResponseModel(success=True, message="Terrorism Rates", data=res)
